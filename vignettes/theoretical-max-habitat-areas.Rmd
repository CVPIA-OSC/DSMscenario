...
We then reduced the multidimensional array of NDWI rasters by selecting the maximum
NDWI at each pixel. We created a buffer of the fall run rearing extents with an estimated channel width to clip the max NDWI raster. A binary filter was applied to the clipped raster for NDWI values greater than a threshold to classify each pixel as water or not water. These
water pixels were multiplied by the pixel area and then all summed to estimate the total
wetted area of the stream.
...
# to calculate other species take fr rearing and scale using rear length ratio between fr and target species
```

## theoretical is bigger than estimated habitat sometimes
No watershed specific instream spawning or rearing habitat data was available for [Bear Creek](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/bear_creek.html), ,  [Paynes Creek](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/paynes_creek.html) [Regional Approximation Method](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/Regional_Approximation.html)

## Rearing
[Cottonwood Creek](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/cottonwood_creek.html), [Cow Creek](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/cow_creek.html), [Mokelumne River](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/mokelumne_river.html), and [Merced River](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/merced_river.html)
...
```{r eval=F}
max_existing_habitat_fr <- purrr::map_dbl(1:31, 
                                       ~max(
                                         DSMhabitat::square_meters_to_acres(
                                           DSMhabitat::fr_fry[.,,])))

max_added_habitat <- (2*5*20)
max_existing_habitat + max_added_habitat

mm <- DSMhabitat::square_meters_to_acres(create_max_area(max_area_df, specie = "fr", life_stage = "rearing"))

DSMscenario::watershed_labels[mm < max_existing_habitat]

mm - max_existing_habitat
